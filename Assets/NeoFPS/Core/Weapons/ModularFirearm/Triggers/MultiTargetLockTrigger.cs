...
namespace NeoFPS.ModularFirearms
{
    ...
    public class MultiTargetLockTrigger : BaseTriggerBehaviour, ITargetingSystem, ITargetLock
    {
        ...
        [SerializeField, AnimatorParameterKey(AnimatorControllerParameterType.Int, true, true), Tooltip("The int animator property key to set based on how many shots are queued")]
...
#pragma warning restore CS0067
        ...
        void AddTarget(Collider target)
        {
            ...
            // Update animator
            if (m_QueueCountHash != -1)
                firearm.animationHandler.SetInteger(m_QueueCountHash, count);
        }
        ...
        void ClearTargets()
        {
            if (m_CurrentLocks.Count > 0)
            {
                ...
                // Update animator
                if (m_QueueCountHash != -1)
                    firearm.animationHandler.SetInteger(m_QueueCountHash, 0);
            }
        }
        ...
        void CheckTargets()
        {
            ...
            if (oldCount != newCount)
            {
                ...
                // Update animator
                if (m_QueueCountHash != -1)
                    firearm.animationHandler.SetInteger(m_QueueCountHash, newCount);
            }
        }
